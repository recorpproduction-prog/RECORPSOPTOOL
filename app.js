...
// GitHub fallback when Cloud Run fails: derive owner/repo from app URL (e.g. user.github.io/REPO -> owner=user, repo=REPO)
function getGitHubSyncRepo() {
    try {
        if (window.SOP_SYNC_REPO && window.SOP_SYNC_REPO.owner && window.SOP_SYNC_REPO.repo)
            return { owner: window.SOP_SYNC_REPO.owner, repo: window.SOP_SYNC_REPO.repo };
        var host = (window.location && window.location.hostname) || '';
        var path = (window.location && window.location.pathname) || '';
        if (/\.github\.io$/i.test(host)) {
            var owner = host.replace(/\.github\.io$/i, '');
            var m = path.match(/^\/([^\/]+)/);
            var repo = m ? m[1] : 'RECORPSOPTOOL';
            if (owner && repo) return { owner: owner, repo: repo };
        }
    } catch (_) {}
    return { owner: 'recorpproduction-prog', repo: 'RECORPSOPTOOL' };
}
...
function saveToGitHub(path, content, message) {
    var token = getGitHubSyncToken();
    if (!token) return Promise.reject(new Error('No GitHub token'));
    var r = getGitHubSyncRepo();
    var apiUrl = 'https://api.github.com/repos/' + r.owner + '/' + r.repo + '/contents/' + path;
    return fetch(apiUrl, { method: 'GET', headers: { 'Accept': 'application/vnd.github.v3+json', 'Authorization': 'token ' + token } })
        .then(function (res) { return res.ok ? res.json() : null; })
        .then(function (existing) {
            var body = { message: message || 'Update ' + path, content: btoa(unescape(encodeURIComponent(JSON.stringify(content, null, 2)))), branch: 'main' };
            if (existing && existing.sha) body.sha = existing.sha;
            return fetch(apiUrl, { method: 'PUT', headers: { 'Accept': 'application/vnd.github.v3+json', 'Authorization': 'token ' + token, 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
        })
...
function applySync(cloudUsers, cloudRequests) {
    ...
    cloudRequests = Array.isArray(cloudRequests) ? cloudRequests : [];
    var localUsers = [];
